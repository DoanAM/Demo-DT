...
#ifndef MW_MWDRILL_HPP_
#define MW_MWDRILL_HPP_
...
std::ostream& operator<<(std::ostream& os, const cadcam::mwDrill& toDump);
...
namespace cadcam
{
/// This class represents a drill tool.
class MW_5AXUTIL_API mwDrill: public mwTypedRevolvedTool
{
	/// @param os is the stream.
	/// @param toDump is this class.
	/// @return The resulting stream.
	friend std::ostream& operator<<(std::ostream& os, const cadcam::mwDrill& toDump);
...
public:
	...
	//##################### Lifecycle #############################################
	/// Default constructor
	explicit mwDrill(const Units units = Units::METRIC);

	/// Constructor
	///
	/// Constructs a drill tool using the given parameters.
	...
	/// @param newArbor is the arbor.
	/// @param newTipAngle is the tip angle of the tool.
	/// @param newShoulderDef is the new shoulder definition.
	/// @param units are the units.
	mwDrill(
		const double& newDiameter,
		const HolderDefinition& newHolder,
	...
	const mwDrill& operator=(const mwDrill& toCopy);
	...
	///  <b>false</b> otherwise.
	bool operator==(const mwDrill& toCompare) const;
	...
	///  <b>true</b> otherwise.
	bool operator!=(const mwDrill& toCompare);
...
protected:
private:
	/// Copies data from mwDrill.
	void Copy(const mwDrill& rToCopy);
