...
//@ret: void 
void set_stock(char* inputStock) 
{

	misc::mwAutoPointer<cadcam::mwTMesh<float>> pStockMesh(new cadcam::mwTMesh<float>(measures::mwUnitsFactory::METRIC));
	misc::mwFileName fileName(inputStock);
	cadcam::mwfSTLTranslator::ReadSTL(fileName, *pStockMesh);
	verifier->ForceDataModel(mwMachSimVerifier::MWV_FM_DEXELBLOCK);
	verifier->SetMesh(pStockMesh);	
}

void new_stock(float init_x, float init_y, float init_z, float end_x, float end_y, float end_z, char* location)
{
	...
	misc::mwstring startName = location; //"C:\\Users\\Minh\\OneDrive\\MasterThesis\\Skripts\\LiveMRS\\StartMesh.stl";
	verifier->GetMesh(&startName);
...
void InitializeToolBehavior(mwMachSimVerifier::mwvToolBehavior& toolBehavior)
{
	toolBehavior.Flute.RemovesMaterial = true;
	toolBehavior.Flute.IsCollisionChecked = false;
	toolBehavior.Flute.RapidCollisionReportSuppressed = true;
	toolBehavior.Flute.SimulateHoles = false;
	toolBehavior.Flute.SimulateThicknessInTurning = false;
	toolBehavior.Flute.IsSafetyDistanceChecked = false;
	toolBehavior.Flute.AddMaterial = false;

	toolBehavior.Shaft.RemovesMaterial = true;
	toolBehavior.Shaft.IsCollisionChecked = true;
	toolBehavior.Shaft.RapidCollisionReportSuppressed = true;
	toolBehavior.Shaft.SimulateHoles = false;
	toolBehavior.Shaft.SimulateThicknessInTurning = false;
	toolBehavior.Shaft.IsSafetyDistanceChecked = false;
	toolBehavior.Shaft.AddMaterial = false;

	toolBehavior.Arbor.RemovesMaterial = false;
	toolBehavior.Arbor.IsCollisionChecked = true;
	toolBehavior.Arbor.RapidCollisionReportSuppressed = true;
	toolBehavior.Arbor.SimulateHoles = false;
	toolBehavior.Arbor.SimulateThicknessInTurning = false;
	toolBehavior.Arbor.IsSafetyDistanceChecked = false;
	toolBehavior.Arbor.AddMaterial = false;

	toolBehavior.Holder.RemovesMaterial = false;
	toolBehavior.Holder.IsCollisionChecked = true;
	toolBehavior.Holder.RapidCollisionReportSuppressed = true;
	toolBehavior.Holder.SimulateHoles = false;
	toolBehavior.Holder.SimulateThicknessInTurning = false;
	toolBehavior.Holder.IsSafetyDistanceChecked = false;
	toolBehavior.Holder.AddMaterial = false;
}

void createTool()
{
	const measures::mwUnitsFactory::Units units = measures::mwUnitsFactory::METRIC;
	cadcam::mwHolderDefinition<double> holder = cadcam::ToolHelper::CreateHolderAsCylinder(5.0, 5.0, units);
	cadcam::mwArborDefinition<double> arbor = cadcam::ToolHelper::CreateArborAsCylinder(5.0, 5.0, units);
	cadcam::mwConstToolPtr endMill = new cadcam::mwEndMill(2.0, holder, 20.0, 5.0, arbor, units);
	verifier->SetTool(endMill, 0);
	mwMachSimVerifier::mwvToolBehavior toolBehavior;
	InitializeToolBehavior(toolBehavior);
	verifier->SetToolBehavior(toolBehavior, false);
}

void cut(float x_start,
	float y_start,
	float z_start,
	float x_end,
	float y_end,
	float z_end)
{
	...
	mwMachSimVerifier::float3d orientation(0.0f, 0.0f, 1.0f);
	mwMachSimVerifier::Frame::Quaternion quat = MATH::OrientationToQuaternion<float>(orientation, 0);
	mwMachSimVerifier::Frame frame1(mwMachSimVerifier::float3d(-5.0f, -5.0f, -5.0f), quat);
	mwMachSimVerifier::Frame frame2(mwMachSimVerifier::float3d(5.0f, 5.0f, 5.0f), quat);
	mwMachSimVerifier::Frame frame3(mwMachSimVerifier::float3d(10.0f, 10.0f, 0.0f), quat);

	//Define the first move
	verifier->SetMoveID(1.0f);
	verifier->Cut(frame1, frame2);

	//Define the second move
	//verifier->SetMoveID(2.0f);
	//verifier->Cut(frame2, frame3);

	//Save the result as an .stl
	misc::mwstring resultName = "C:\\Users\\Minh\\OneDrive\\MasterThesis\\Skripts\\LiveMRS\\ResultMesh.stl";
	verifier->GetMesh(&resultName);

	//Everything worked!
	misc::mwCout << "Lesson02 - Success";
}