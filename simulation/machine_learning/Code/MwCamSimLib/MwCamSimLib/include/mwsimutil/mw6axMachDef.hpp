...
#ifndef MW_MW6AXMACHDEF_HPP_
#define MW_MW6AXMACHDEF_HPP_
...
namespace post
{
	...
	class MW_5AXUTIL_API mw6axMachDef : public mwMachDef
	{
	public:
		enum AxisType
		{
		...
		*/
		...
		/*!
			...
			\param vThirdAngle defines third rotary axis value for 5+1 machines
		...
		*/
		...
		//! Copy constructor
		mw6axMachDef(const mw6axMachDef& toCopy);
		//! copy operator
		const mw6axMachDef& operator = (const mw6axMachDef& toCopy);
		//! Comparison operator
		/* Compares two mw6axMachDef objects
			\return true it the compared objects are equal member by member, 
			\false otherwise
		*/
		const bool operator == (const mw6axMachDef& toCompare) const;
		...
		*/
		...
		inline void SetRotAxisDir0(const vector3d& rotAxisDir);
		...
		*/
		...
		//! Gets third rotary value (in degrees)
		/*! 			
			\returns the RotAxisMaxLimit2
		*/
		const double GetThirdAngle() const;
		//! Sets third rotary value (in degrees)
		void SetThirdAngle(const double angle);
	...
	typedef misc::mwAutoPointer<mw6axMachDef> mw6axMachDefPtr;
}
